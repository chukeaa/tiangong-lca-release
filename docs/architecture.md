...
- 不为目录对称创建空运行时抽象；
- Dataset Transformation 只实现已确认的 `process.weighted-aggregate.v0`，不通过通用 patch 或表达式扩大语义；Publication 只实现已确认的 exact-plan-hash、actor-scoped 平台 adapter、状态 mapping，以及 Database-owned Portal LCIA V3 package publish 和 projection finalize/readback/revoke 契约。
