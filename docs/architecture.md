...
- Publish Plan：绑定 Candidate/scope/target intent、尚未授权且不可原地修改的执行前计划；
...
Publication 只消费不可变 Candidate v2。当前本地 planner 支持 Unit Process、Result、Both 和 exact include/exclude；它计算 forward closure、transitive reverse pruning 和 reference-complete effective set，并冻结未授权 Publish Plan。

远程 target inspection、UUID + Version 内容分类、state code mapping、approval、事务写入、恢复和 independent readback 尚未实现，任何远程发布动作保持 fail closed。Publication 不拥有内容变换或包重建。
...
- 不为目录对称创建空运行时抽象；
- Publication 只实现已确认的本地确定性规划；Dataset Transformation 在详细设计完成前只保留文档边界，不添加推测性执行代码。
...
- Publication 已实现 Candidate v2 handoff、范围解析、prepared Publish Plan、CLI 和测试；Dataset Transformation 当前尚无执行入口。
